...
if (isset($_REQUEST['searchPhrase']) )
  {
    $search=trim($_REQUEST['searchPhrase']);
  	$where.= " AND ( nombre LIKE '".$search."%' OR  nivel LIKE '".$search."%' OR sha_pass LIKE '".$search."%') "; 
	}
...
$sql="SELECT id, nombre, sha_pass, nivel, sessionkey FROM usuarios WHERE $where ORDER BY $order_by $limit";
if($bbdd_tipo=="sqlite"){
	...
	$numeros = "SELECT count(*) FROM usuarios WHERE $where";
